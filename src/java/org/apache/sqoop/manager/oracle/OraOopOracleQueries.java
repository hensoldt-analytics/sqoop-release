  ...
  public static void mergeTable(Connection connection, OracleTable targetTable,
    ...
    for (int idx = 0; idx < oracleTableColumns.size(); idx++) {
      ...
      if (insertClause.length() > 0) {
        ...
        insertClause.append(String.format("target.%s", OracleUtils.escapeIdentifier(columnName)));

      if (valuesClause.length() > 0) {
        valuesClause.append(",");
      }
      valuesClause.append(String.format("source.%s", OracleUtils.escapeIdentifier(columnName)));
